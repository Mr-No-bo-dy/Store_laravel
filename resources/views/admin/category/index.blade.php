...
<div class="wrapper mx-20">
    <h1>Categories</h1>
    <div><a class="text-blue-500 bg-green-300" href="{{ route('admin.categoryCreate') }}">Creatre</a></div>

    @foreach ($categories as $category)
    <div class="mt-4 @if(!is_null($category->deleted_at)) bg-red-300 @endif">
        <p><b>ID: {{ $category->id }}</b></p>
        <p>Name: {{ $category->name }}</p>
        <div><a class="text-blue-500 bg-blue-300" href="{{ route('admin.categoryShow', $category->id) }}">Show</a></div>
    ...
    <form action="{{ route('admin.categoryRestore') }}" method="post">
        @csrf
        @method('patch')
